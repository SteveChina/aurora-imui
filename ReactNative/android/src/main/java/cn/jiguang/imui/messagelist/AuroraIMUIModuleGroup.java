...
import static cn.jiguang.imui.messagelist.viewmanager.ReactMsgListManagerGroup.RCT_REMOVE_ALL_MESSAGE_ACTION;
import static cn.jiguang.imui.messagelist.viewmanager.ReactMsgListManagerGroup.RCT_REMOVE_MESSAGE_ACTION;

public class AuroraIMUIModuleGroup extends ReactContextBaseJavaModule {

    private final String REACT_MSG_LIST_MODULE = "AuroraIMUIModuleGroup";
    public static final String RCT_MESSAGE_LIST_LOADED_ACTION = "cn.jiguang.imui.messagelist.intent.messageLoaded";

    private static final String MESSAGE_LIST_DID_LOAD_EVENT = "IMUIMessageListDidLoadGroup";
    public static final String GET_INPUT_TEXT_EVENT = "getInputTextGroup";

    public AuroraIMUIModuleGroup(ReactApplicationContext reactContext) {
        super(reactContext);
    }
    ...
    public void appendMessages(ReadableArray messages) {
        ...
        EventBus.getDefault().post(new MessageEvent(rctMessages, ReactMsgListManagerGroup.RCT_APPEND_MESSAGES_ACTION));
    }
    ...
    public void appendCustomMessage(ReadableMap message) {
        ...
        array[0] = rctMessage;
        EventBus.getDefault().post(new MessageEvent(array, ReactMsgListManagerGroup.RCT_APPEND_MESSAGES_ACTION));
    }

    @ReactMethod
    public void updateMessage(ReadableMap message) {
        RCTMessage rctMessage = configMessage(message);
        EventBus.getDefault().post(new MessageEvent(rctMessage, ReactMsgListManagerGroup.RCT_UPDATE_MESSAGE_ACTION));
    }
    ...
    public void insertMessagesToTop(ReadableArray messages) {
        ...
        EventBus.getDefault().post(new MessageEvent(rctMessages, ReactMsgListManagerGroup.RCT_INSERT_MESSAGES_ACTION));
    }
    ...
    private RCTMessage configMessage(ReadableMap message) {
        Log.d("AuroraIMUIModuleGroup", "configure message: " + message);
        ...
        Log.d("AuroraIMUIModuleGroup", "fromUser: " + rctUser);
    ...
    public void compressImage(ReadableMap map, Callback callback) {
        ...
        try {
            ...
            if (file.exists() && file.isFile()) {
                ...
                Log.d("AuroraIMUIModuleGroup", "compress image result path: " + resultPath);
